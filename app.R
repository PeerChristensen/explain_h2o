      ...
      tabItem(tabName = "local_expl",
              fluidRow(
                box(title= "Individual explanations",width=8,
                    withSpinner(plotOutput("lime_plot"),color=spinner_colour)),
                box(title = "Options",width=4,
                    selectInput("lime_cust",label="CustomerID",
                                choices = prediction_table$CustomerID,
                                selected = sample(prediction_table$CustomerID,1),
                                selectize = T),
                    sliderInput("lime_n_feat",label="Number of features",
                                min = 1,
                                max = 15,
                                value = 5,
                                step = 1))
  ...
  if (input$tabs=="cm") {
   ...
   # FEATURE IMPORTANCE
    ...
    if (input$tabs == "local_expl") {
      ...
      observeEvent(c(input$lime_cust,input$lime_n_feat),{
      ...
      output$lime_plot <- renderPlot({
       plot_features(explanation) +
          ...
          scale_fill_manual(values = c(line_colour,"grey45")) +
          theme(plot.background = element_rect(fill=back_colour,colour =back_colour),
                panel.background =  element_rect(fill=back_colour,colour =back_colour),
                panel.grid = element_line(size=.05,),
                axis.title = element_blank(),
